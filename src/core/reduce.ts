...
function baseReduce(state: AppState, command: Command): AppState {
  switch (command.type) {
    ...
    case 'template.saveNew':
      // Saving becomes the active template — you're now "on" it.
      return {
        ...state,
        savedTemplates: [
          ...state.savedTemplates,
          { id: command.id, name: command.name, slides: command.slides },
        ],
        activeTemplate: command.id,
      }
    ...
    case 'template.remove':
      return {
        ...state,
        savedTemplates: state.savedTemplates.filter((t) => t.id !== command.id),
        // Deleting the one you're on leaves you on nothing, rather than pointing
        // at a template that no longer exists.
        activeTemplate: state.activeTemplate === command.id ? null : state.activeTemplate,
      }
    case 'template.setActive':
      return { ...state, activeTemplate: command.id }
    case 'slideshow.save':
      return {
        ...state,
    ...
    case 'sound.playTag':
      return {
        ...state,
        soundTagCue: { tag: command.tag, mode: command.mode },
        soundTagCueNonce: state.soundTagCueNonce + 1,
        music: { ...state.music, duck: 1 },
      }
